    ...
    with tf.compat.v1.Session(config=cfg) as sess:
        ...
        for fn in data_list:
            ...
            if (boxes is not None) and (len(boxes[0]) > 0):
                ...
                cv2.imwrite('/content/drive/MyDrive/DS_store/DS_train/Det_train/Detection/detect_text/test_detection' + fn[:len(fn) - 4] + '_img.jpg', bgr_input)


if __name__ == '__main__':
    test('/content/drive/MyDrive/DS_store/DS_train/Det_train/test', 
         '/content/drive/MyDrive/DS_store/DS_train/Det_train/test.list')
